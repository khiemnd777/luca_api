...
ALTER TABLE order_item_processes
  ADD COLUMN IF NOT EXISTS process_name_norm text GENERATED ALWAYS AS (lower(unaccent_immutable(process_name))) STORED,
  ADD COLUMN IF NOT EXISTS section_name_norm text GENERATED ALWAYS AS (lower(unaccent_immutable(section_name))) STORED;
