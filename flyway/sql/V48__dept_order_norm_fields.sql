...
ALTER TABLE orders
  ...
  ADD COLUMN IF NOT EXISTS customer_name_norm text GENERATED ALWAYS AS (lower(unaccent_immutable(customer_name))) STORED,
  ADD COLUMN IF NOT EXISTS code_latest_norm text GENERATED ALWAYS AS (lower(unaccent_immutable(code_latest))) STORED;
